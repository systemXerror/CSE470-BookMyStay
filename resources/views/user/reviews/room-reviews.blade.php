...
<head>
    ...
    <title>Room {{ $room->room_number }} Reviews - BookMyStay</title>
...
<body>
    ...
    <div class="bg-light py-4">
        <div class="container">
            <div class="row align-items-center">
                <div class="col-md-3">
                    @if($room->image)
                        <img src="{{ $room->image }}" alt="Room {{ $room->room_number }}" class="img-fluid rounded">
                    @else
                ...
                <div class="col-md-6">
                    <h2 class="mb-2">Room {{ $room->room_number }} - {{ $room->type }}</h2>
                    ...
                    <p class="text-muted mb-0">
                        ...
                        ${{ number_format($room->price_per_night, 2) }} per night
                    </p>
                </div>
                <div class="col-md-3 text-md-end">
                    <div class="text-center">
                        <div class="star-rating mb-2">
                            @for($i = 1; $i <= 5; $i++)
                                <i class="fas fa-star{{ $i <= $room->rating ? '' : '-o' }} fa-2x"></i>
                            @endfor
                        </div>
                        <h4 class="mb-1">{{ number_format($room->rating, 1) }}/5</h4>
                        <p class="text-muted mb-0">{{ $room->total_reviews }} reviews</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="container py-5">
        ...
        <div class="row mb-4">
            <div class="col-md-6">
                <a href="{{ route('user.room.detail', ['hotel' => $room->hotel->id, 'room' => $room->id]) }}" class="btn btn-outline-secondary">
                    <i class="fas fa-arrow-left me-2"></i>Back to Room Details
                </a>
            </div>
            <div class="col-md-6 text-md-end">
                @if(auth()->check() && !$room->reviews()->where('user_id', auth()->id())->exists())
